...
fn main() {
    ...
    let cube_mesh = cgl::utils::load_mesh("assets/cube.glb");
    let sphere_mesh = cgl::utils::load_mesh("assets/sphere.glb");
...
impl ReflectiveObjectShader {
    pub fn new() -> Self {
        let vertex_shader_source = include_str!("../shaders/pos_norm.vert");
        let fragment_shader_source = include_str!("../shaders/cubemap_reflection.frag");
...
impl BackgroundShader {
    pub fn new() -> Self {
        let vert = include_str!("../shaders/cubemap_background.vert");
...
impl ObjectShader {
    pub fn new() -> Self {
        let vertex_shader_source = include_str!("../shaders/pos_norm.vert");
        let fragment_shader_source = include_str!("../shaders/fixed_pointlight1.frag");
        let vert = cgl::compile_shader(gl::VERTEX_SHADER, vertex_shader_source).unwrap();
        let frag = cgl::compile_shader(gl::FRAGMENT_SHADER, fragment_shader_source).unwrap();
        let program = cgl::create_vert_frag_prog(vert, frag).unwrap();
        let mvp_location = cgl::get_location(program, "mvp").unwrap();
